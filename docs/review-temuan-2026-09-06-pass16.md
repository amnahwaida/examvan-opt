...
- Full production verification of system-app multipart upload/R2 metadata cleanup remains. System-app list/delete/upload and SMTP test routes are implemented; upload fails closed when R2 or metadata persistence is unavailable, and delete cleans metadata then reports R2 cleanup failures explicitly. The legacy `/admin/api/pengawas/state` route is registered with a stable authenticated zero-state response; live aggregate metrics still require a product/data-source contract.
