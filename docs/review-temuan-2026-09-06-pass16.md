...
- Full production verification of system-app multipart upload/R2 metadata cleanup and the pengawas-state endpoint remain. System-app list/delete/upload and SMTP test routes are now implemented; upload fails closed when R2 or metadata persistence is unavailable.
